#!/usr/bin/env python3
"""
Build_FallSoftball_History_json.py
 
Aggregates historical AllMOSports fall softball ratings (2012-2024) AND
...
Adapted from Build_BoysSoccer_History_json.py -- schema for both
sources is IDENTICAL to boys soccer's (confirmed against real 2012
files for fall softball too), so this is mostly a find-and-replace of
URLs/sport name rather than new logic. Unlike girls volleyball,
off_rating/off_rank/def_rating/def_rank ARE kept here -- fall softball's
raw ratings source has real, meaningful values for those fields.
 
Sources per year:
  1. Ratings:  https://raw.githubusercontent.com/AllMOSports/Fall_Softball_Ratings_{year}/main/fall_softball_ratings_{year}.json
  2. Records:  https://raw.githubusercontent.com/AllMOSports/All_MO_Sports-Data/main/output/mshsaa_historical_records/fall_softball/fall_softball_{year}.json
...
Output shape (fall_softball_history.json):
{
  "sport": "fall_softball",
  "years": [2012, 2013, ..., 2024],
  "teams": {
    "<slug>": {
      "school": "Troy Buchanan",
      "history": {
        "2012": {
          "ovr_rank": 1, "classification": 4, "district": 8,
          "ovr_rating": 13.76, "off_rating": 6.5, "off_rank": 1,
          "def_rating": 7.26, "def_rank": 2,
...
Usage:
  python Build_FallSoftball_History_json.py
"""
...
YEAR_REPO_OVERRIDES = {
    # 2013: "Fall_Softball_Ratings_2012",
}
 
def repo_and_file_for_year(year: int):
    repo = YEAR_REPO_OVERRIDES.get(year, f"Fall_Softball_Ratings_{year}")
    fname = f"fall_softball_ratings_{year}.json"
    return repo, fname
...
RECORDS_URL_TEMPLATE = (
    ...
    "output/mshsaa_historical_records/fall_softball/fall_softball_{year}.json"
)
 
SCHOOLS_JSON_PATH = "output/schools.json"
ALIASES_JSON_PATH = "Aliases.json"
OUTPUT_PATH = "fall_softball_history.json"
    ...
    for year in YEARS:
        ...
        for team in ratings_data.get("teams", []):
            ...
            if rec_match:
                ...
                # a literal 0 in that column). Fall softball only goes back
                # to 2012, where football's win_pct was already reliable --
                # but deriving it ourselves costs nothing and removes any
                # risk of a similar per-year quirk showing up unnoticed here.
