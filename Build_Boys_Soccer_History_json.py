#!/usr/bin/env python3
"""
Build_BoysSoccer_History_json.py
 
Aggregates historical AllMOSports boys soccer ratings (2012-2024) AND
...
Adapted from Build_Football_History_json.py -- schema for both sources
is IDENTICAL to football's (confirmed against real 2012 files for both
sports), so this is mostly a find-and-replace of URLs/year range rather
than new logic.
 
Sources per year:
  1. Ratings:  https://raw.githubusercontent.com/AllMOSports/Boys_Soccer_Ratings_{year}/main/boys_soccer_ratings_{year}.json
  2. Records:  https://raw.githubusercontent.com/AllMOSports/All_MO_Sports-Data/main/output/mshsaa_historical_records/boys_soccer/boys_soccer_{year}.json
...
Output shape (boys_soccer_history.json):
{
  "sport": "boys_soccer",
  ...
  "teams": {
    "<slug>": {
      ...
      "history": {
        "2012": {
          "ovr_rank": 1, "classification": 3, "district": 2,
          "ovr_rating": 6.06, "off_rating": 3.13, "off_rank": 1,
          "def_rating": 2.94, "def_rank": 3,
...
Usage:
  python Build_BoysSoccer_History_json.py
"""
...
# -- but football had that surprise (2011/2019 bundled into the 2010 repo),
# so if a fetch below 404s, check whether that year got bundled into a
# neighboring repo before assuming the data doesn't exist.
YEAR_REPO_OVERRIDES = {
    # 2013: "Boys_Soccer_Ratings_2012",
}
 
def repo_and_file_for_year(year: int):
    repo = YEAR_REPO_OVERRIDES.get(year, f"Boys_Soccer_Ratings_{year}")
    fname = f"boys_soccer_ratings_{year}.json"
    return repo, fname
...
RECORDS_URL_TEMPLATE = (
    ...
    "output/mshsaa_historical_records/boys_soccer/boys_soccer_{year}.json"
)
...
OUTPUT_PATH = "boys_soccer_history.json"
