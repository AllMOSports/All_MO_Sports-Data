#!/usr/bin/env python3
"""
Build_BoysBasketball_History_json.py
 
Aggregates historical AllMOSports boys basketball ratings (2012-2024)
...
Adapted from Build_FallSoftball_History_json.py -- schema for both
sources is IDENTICAL to fall softball's (confirmed against real 2012
files), so this is mostly a find-and-replace of URLs/sport name.
 
ONE REAL DIFFERENCE FROM EVERY OTHER SPORT SO FAR: the ratings repo
naming does NOT follow the simple "Sport_Ratings_{year}" pattern.
Boys basketball repos are named with the full season span, e.g.
"Boys_Basketball_Ratings_2012-2013" for the 2012 season -- but the
FILENAME inside that repo is still just "boys_basketball_ratings_2012.json"
(single year, matching every other sport). repo_and_file_for_year()
below builds the season-span repo name; if a fetch 404s, double check
this pattern holds for that particular year before assuming the data
doesn't exist.
 
Sources per year:
  1. Ratings:  https://raw.githubusercontent.com/AllMOSports/Boys_Basketball_Ratings_{year}-{year+1}/main/boys_basketball_ratings_{year}.json
  2. Records:  https://raw.githubusercontent.com/AllMOSports/All_MO_Sports-Data/main/output/mshsaa_historical_records/boys_basketball/boys_basketball_{year}.json
...
Output shape (boys_basketball_history.json):
{
  ...
  "teams": {
    "<slug>": {
      ...
      "history": {
        "2012": {
          "ovr_rank": 1, "classification": 3, "district": 1,
          "ovr_rating": 40.82, "off_rating": 26.28, "off_rank": 2,
          "def_rating": 14.54, "def_rank": 24,
...
Usage:
  python Build_BoysBasketball_History_json.py
"""
...
YEAR_REPO_OVERRIDES = {
    # If a specific year breaks the "{year}-{year+1}" pattern, hardcode
    # its actual repo name here, e.g.:
    # 2019: "Boys_Basketball_Ratings_2019-2020_v2",
}
 
def repo_and_file_for_year(year: int):
    repo = YEAR_REPO_OVERRIDES.get(year, f"Boys_Basketball_Ratings_{year}-{year + 1}")
    fname = f"boys_basketball_ratings_{year}.json"
    return repo, fname
...
RECORDS_URL_TEMPLATE = (
    ...
    "output/mshsaa_historical_records/boys_basketball/boys_basketball_{year}.json"
)
 
SCHOOLS_JSON_PATH = "output/schools.json"
ALIASES_JSON_PATH = "Aliases.json"
OUTPUT_PATH = "boys_basketball_history.json"
    ...
    for year in YEARS:
        ...
        for team in ratings_data.get("teams", []):
            ...
            if rec_match:
                ...
                # a literal 0 in that column). Boys basketball only goes
                # back to 2012, where football's win_pct was already
                # reliable -- but deriving it ourselves costs nothing and
                # removes any risk of a similar per-year quirk here too.
